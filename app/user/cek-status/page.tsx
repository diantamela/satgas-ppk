                ...
                <Card className={styles.statusCard}>
                  <CardContent className="pt-5 pb-5">
                    ...
                    <div className={styles.infoGrid}>
                      ...
                      <div>
                        <p className={styles.value}>
                          <span className={styles.label}>Kesimpulan & Rekomendasi: </span>
                          {reportStatus.latestResult ? 
                            (reportStatus.latestResult.sessionInterimConclusion || 
                             reportStatus.latestResult.recommendedImmediateActions ||
                             reportStatus.latestResult.recommendedActionsDetails ||
                             'Hasil investigasi tersedia') :
                            (reportStatus.recommendation || 'Belum ada rekomendasi')}
                          {reportStatus.latestResult && (
                        ...
                        <p className={styles.value}>
                          <span className={styles.label}>Status Kasus: </span>
                          {reportStatus.latestResult?.caseStatusAfterResult ? 
                            (reportStatus.latestResult.caseStatusAfterResult === 'UNDER_INVESTIGATION' ? 'Dalam Investigasi' :
                             reportStatus.latestResult.caseStatusAfterResult === 'EVIDENCE_COLLECTION' ? 'Pengumpulan Bukti' :
                             reportStatus.latestResult.caseStatusAfterResult === 'STATEMENT_ANALYSIS' ? 'Analisis Pernyataan' :
                             reportStatus.latestResult.caseStatusAfterResult === 'PENDING_EXTERNAL_INPUT' ? 'Menunggu Input Eksternal' :
                             reportStatus.latestResult.caseStatusAfterResult === 'READY_FOR_RECOMMENDATION' ? 'Siap untuk Rekomendasi' :
                             reportStatus.latestResult.caseStatusAfterResult === 'CLOSED_TERMINATED' ? 'Ditutup/Terminated' :
                             reportStatus.latestResult.caseStatusAfterResult === 'FORWARDED_TO_REKTORAT' ? 'Diteruskan ke Rectorat' :
                             reportStatus.latestResult.caseStatusAfterResult) :
                            (reportStatus.finalDecision ? 
                              (reportStatus.finalDecision === 'APPROVED' ? 'Disetujui' :
                               reportStatus.finalDecision === 'REJECTED' ? 'Ditolak' :
                               reportStatus.finalDecision === 'FOLLOW_UP' ? 'Tindak Lanjut' : 
                               reportStatus.finalDecision) :
                              (reportStatus.recommendationStatus === 'PENDING' ? 'Menunggu' :
                               reportStatus.recommendationStatus === 'SUBMITTED' ? 'Disubmit' :
                               reportStatus.recommendationStatus === 'APPROVED' ? 'Disetujui' :
                               reportStatus.recommendationStatus === 'REJECTED' ? 'Ditolak' :
                               reportStatus.recommendationStatus === 'IMPLEMENTED' ? 'Diimplementasikan' :
                               reportStatus.recommendationStatus || 'Belum ditentukan'))}
                        </p>
                      </div>
                    </div>
                  </CardContent>
